...
ZEUS_CHAIN_WAIT_SECONDS = 15

load_zeus = true

if ENV['IDE_PROCESS_DISPATCHER']
  puts 'Debugger active - bypassing zeus.'
  load_zeus = false
elsif !File.exists?('.zeus.sock')
  puts 'Zeus not active - chaining zeus...'
  `/usr/local/bin/zsh -c '~/bin/it zeus'`
  sleep(ZEUS_CHAIN_WAIT_SECONDS)
  load_zeus = true
end
